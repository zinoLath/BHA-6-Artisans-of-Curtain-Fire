...
{
    float4 texColor = GaussianSampler25(input.uv + offset,radius);//获得均值颜色，25点
    //float4 texColor = ScreenTexture.Sample(ScreenTextureSampler, input.uv);
    texColor.rgb *= 0.0f;
    float4 texColor2 = ScreenTexture.Sample(ScreenTextureSampler, input.uv);
    texColor.a *= alpha;
    texColor = lerp(texColor,texColor2,texColor2.a);
